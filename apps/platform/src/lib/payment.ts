...
async function callCdp(
  ...
  try {
    return await fetch(
      `https://api.cdp.coinbase.com/platform/v2/x402/${path}`,
      {
        method: "POST",
        headers: {
    ...
    throw new ApiError(
      timeoutStatus,
      `Payment ${path} did not respond in time`,
    );
  }
}
...
export async function verifyPayment(
  ...
  if (!response.ok)
    throw new ApiError(402, "Payment authorization could not be verified");
  const verified = (await response.json()) as {
    isValid?: boolean;
    payer?: string;
  };
  if (!verified.isValid)
    throw new ApiError(402, "Payment authorization is not valid");
  return verified;
}
...
export async function settlePayment(
  ...
  if (!response.ok)
    throw new ApiError(502, "Payment settlement could not be completed");
